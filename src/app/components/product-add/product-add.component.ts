...
import { SessionStorageService } from '../../services/session-storage.service';  // Importa el servicio
...
export class ProductAddComponent implements OnInit {
  ...
  id: number = 0;
  code: string = 'ACEITE' + Math.floor(Math.random() * 100) + 1;
  name: string = '';
  description: string = '';
  price: number = 0;
  urlImage: string = '';
  ...
  addProduct(): void {
    ...
    if (!userId) {
      this.toastr.error('User not logged in!', 'Error', { timeOut: 3000 });
      return;
    }
    ...
    formData.append('dateUpdated', this.dateUpdated instanceof Date ? this.dateUpdated.toISOString() : new Date().toISOString());
    ...
    this.productService.createProduct(formData).subscribe(
      data => {
        console.log(data);
        if (this.id == 0) {
          this.toastr.success('Producto guardado con éxito!', 'OK', { timeOut: 3000 });
        } else {
          this.toastr.success('Producto actualizado con éxito!', 'OK', { timeOut: 3000 });
        }
